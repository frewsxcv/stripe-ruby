  ...
  class StripeClientTest < Test::Unit::TestCase
    ...
    context "#execute_request" do
      ...
      context "error handling" do
        ...
        should "raise InvalidRequestError when error is a string with value 'invalid_request'" do
        ...
        should "raise OAuthError when error is a string" do
          stub_request(:post, "#{Stripe.connect_base}/oauth/token").
            to_return(body: JSON.generate({
              error: "invalid_grant",
          ...
          client = StripeClient.new
          opts = {api_base: Stripe.connect_base}
          e = assert_raises Stripe::OAuthError do
            client.execute_request(:post, '/oauth/token', opts)
          end

          assert_equal(400, e.http_status)
          assert_equal('invalid_grant', e.type)
